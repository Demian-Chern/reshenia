...
void SHA512::padMessage(const uint8_t* message, size_t length,
                        uint8_t** paddedMessage, size_t* paddedLength) {
    size_t bitLength = length * 8;
    size_t padLength = ((length + 16 + 127) / 128) * 128;

    *paddedMessage = new uint8_t[padLength];
    memset(*paddedMessage, 0, padLength);
    memcpy(*paddedMessage, message, length);
    (*paddedMessage)[length] = 0x80;

    for (int i = 0; i < 8; i++) {
        (*paddedMessage)[padLength - 1 - i] = (bitLength >> (i * 8)) & 0xFF;
    }
    *paddedLength = padLength;
}

std::string SHA512::digestToHex() {
...
std::string SHA512::compute(const std::string& input) {
    uint8_t* padded = nullptr;
    size_t paddedLen = 0;

    padMessage((const uint8_t*)input.c_str(), input.length(), &padded, &paddedLen);
